define([
	...
	'/_studio/Modules/TriggerSources/EnvelopeGenerator/EnvelopeGenerator.js'
		...
		return {
			title : 'Envelope Generator to trigger Amp',
			description : 'This rack shows triggering. An Envelope Generator can be started, and triggers an attack-release-decay cycle.'
				...
				+'The EG controls the level/volume/amplitude of the sound (via the gain node), so the outcome is that the volume starts from the min level, rises to the max level, decreases to the sustain level, then lingers until release, then decreases to the min level',
			rackData : {
				rows : [
					{
						modules: [
